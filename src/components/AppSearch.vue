...
import * as docsApi from "@/api/docsApi.ts";
            ...
            <div v-show="searchActive"
                ...
                <div class="relative w-full p-4">
                    <input type="text" v-model="searchValue"
                        class="w-full p-2 text-black border border-sky-500 dark:text-white" ref="SearchInput" />
                    ...
                    <div class="pb-1 text-xs text-gray-500">
                        <div v-show="!searchValue.trim()">
                            <div v-if="searchItemLength">
                                历史记录
                                <span class="float-right cursor-pointer hover:text-red-400" @click="
                                    () => {
                                        clearSearchHistory();
                                    }
                                ">清除</span>
                        ...
                        <div v-show="searchValue.trim()">
                            ...
                            <span class="float-right">{{ searchItemIndex + 1 }}/{{ searchItemLength }}</span>
                        </div>
                    </div>
                    <!-- 搜索结果 / 搜索历史 -->
                    <ul class="space-y-2 scrollbar max-h-[calc(100vh-17rem)] pr-2" ref="ResaultList">
                        <li v-for="(item, index) in resaultItems" :key="item.url" class="my-2 hover:cursor-pointer">
                            <div :to="item.url ? item.url : '/'"
                                class="relative px-3 py-2 border border-gray-200 rounded-md dark:border-gray-600" :class="
                                    index === searchItemIndex
                                        ? 'bg-blue-400 text-white dark:bg-opacity-80'
                                        : 'hover:bg-blue-300 hover:text-white'
                                " @click="item.url && linkTo(item.name, item.url);">
                                <!-- 左侧类型 -->
                                ...
                                <div class="flex flex-col flex-grow w-full px-8">
                                    ...
                                    <span v-show="searchValue" class="text-xs truncate opacity-70">{{ item.url }}</span>
                                    <span v-show="!searchValue.trim()" class="text-xs opacity-70">{{
                                ...
                                <!-- 右侧操作 -->
                                <div class="absolute -translate-y-1/2 top-1/2 right-2 hover:text-red-400 w-fit">
                                    <CloseIcon v-show="!searchValue.trim()" class="w-6"
                ...
                <!-- footer -->
                <div class="flex items-center justify-between p-4 text-xs border-t dark:border-t-gray-500">
                    <span class="font-bold text-blue-500 text-[1rem]">张航</span>
                    <ul v-if="isPc" class="flex gap-4">
                        <li v-for="key in keys" :key="key.name">
                            <span class="inline-block p-1 mr-1 transform scale-75 border rounded-lg">{{ key.name }}</span>
                            <span>{{ key.tip }}</span>
                        </li>
                    </ul>
                </div>
