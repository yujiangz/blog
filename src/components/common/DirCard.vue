...
const childCardEnter = (el: Element) => {
    ...
    gsap.from(el.lastElementChild, { x: "50%", opacity: 0, duration: 0.2 });
};
...
<template>
    <div class="flex overflow-hidden rounded-lg" :style="style.card">
        ...
        <div class="relative flex flex-col justify-between basis-5/12" :style="style.leftWrapper">
            <RandomSvg v-if="svg" class="absolute -z-[1] top-0 left-0 w-full h-full stroke-[.5em]" :style="style.svg" />
        ...
        <ul class="w-full max-h-[100%-2rem] pl-2 pr-4 my-4 text-sm basis-7/12 scrollbar">
        ...
        <!-- 子卡片 -->
        <Transition @enter="childCardEnter" @leave="childCardLeave">
            <div v-if="childCard" class="absolute inset-0 flex h-full overflow-hidden ">

                <!-- 返回按钮 -->

                <div class="flex-grow-0 flex-shrink-0 h-full pt-2 overflow-hidden text-red-300 cursor-pointer bg-slate-300 dark:bg-slate-700 basis-6 hover:text-red-400"
                    @click="childCard = null">
                    <span>
                        <EnterIcon class="w-full rotate-180" />
                    </span>
                    <span class="block w-24 text-sm leading-6 truncate origin-bottom-left rotate-90 -translate-y-6">{{
                        dir.name }}</span>
                </div>

                <!-- 右侧子卡片 -->

                <DirCard :dir="childCard" class="flex-grow h-full rounded-l-none bg-slate-300 dark:bg-slate-700" />
            </div>
        </Transition>
    </div>
</template>
