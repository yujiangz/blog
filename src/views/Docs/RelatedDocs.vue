    ...
    <div v-if="dir" class="relative p-4 select-none">
        ...
        <ul>
            <li v-for="item in dir.children" :key="item.name" class="truncate hover:text-sky-500">
                <!-- 文件夹 -->
                <div v-if="item.type === 'dir'" class="p-1 cursor-pointer" @click="setChildDir(item)">
                    <span class="">
                ...
                <RouterLink v-else :to="item.url" class="block p-1">
                    <span class="inline-block w-2 h-1 mx-1 mb-[.2em] rounded-full bg-sky-500"></span>
                    {{ item.name }}
                </RouterLink>
            </li>
        </ul>
